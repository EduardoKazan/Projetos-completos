...
{
  // Only allow a maximum of ESP_NOW_MAX_DATA_LEN characters in the message + a null terminating byte
  char buffer[ESP_NOW_MAX_DATA_LEN + 1];
  int msgLen = dataLen > ESP_NOW_MAX_DATA_LEN ? ESP_NOW_MAX_DATA_LEN : dataLen;
...
void loop() {
  ...
  if (reading == LOW) {
    ...
    if ((currentTime - lastDebounceTime) > debounceDelay) {
      // Detect the transition from button not pressed to pressed
      if (!buttonDown) {
        buttonDown = true;
